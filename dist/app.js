...
async function startCloudShare(media, profileId) {
  ...
  const transceivers = [];
  transceivers.push(pc.addTransceiver(videoTrack, { direction:'sendonly', sendEncodings:streamEncodings(profileId) }));
  if (audioTrack) transceivers.push(pc.addTransceiver(audioTrack, { direction:'sendonly' }));
  const offer = await pc.createOffer();
  await pc.setLocalDescription(offer);
  const connected = waitForConnected(pc);
  // Use Cloudflare's auto-discovery flow and send the browser's applied local
  // description verbatim. This avoids hand-building track descriptors that can
  // become inconsistent with the SDP generated by the browser.
  const localDescription = pc.localDescription?.toJSON
    ? pc.localDescription.toJSON()
    : { type: pc.localDescription?.type || 'offer', sdp: pc.localDescription?.sdp || offer.sdp };
  ...
  if (!response.sessionDescription) throw new Error('Cloudflare Realtime did not return an SDP answer.');
  await pc.setRemoteDescription(response.sessionDescription);
  await connected;

  // Cloudflare assigns globally unique track names. Those returned names — not
  // the browser MediaStreamTrack.id values — are what other sessions must pull.
  const publishedTracks = Array.isArray(response.tracks) ? response.tracks : [];
  const videoMid = transceivers[0]?.mid;
  const audioMid = audioTrack ? transceivers[1]?.mid : null;
  const videoPublished = publishedTracks.find(t => t.mid === videoMid) || publishedTracks[0];
  const audioPublished = audioTrack
    ? (publishedTracks.find(t => t.mid === audioMid) || publishedTracks.find(t => t.trackName !== videoPublished?.trackName))
    : null;
  if (!videoPublished?.trackName) throw new Error('Cloudflare Realtime did not return a published video track name.');
  ...
  const ann = {
    ...
    mode:'cloud', sessionId:session.sessionId, videoTrackName:videoPublished.trackName, audioTrackName:audioPublished?.trackName || null,
    profile:profileId, audio:Boolean(audioPublished?.trackName),
  };
